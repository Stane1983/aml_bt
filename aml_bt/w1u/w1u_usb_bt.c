...
#define AML_BT_FIRMWARE_NAME        "aml/w1u_bt_fw_usb.bin"
#define AML_BT_FIRMWARE_FT_NAME     "aml/w1u_bt_fw_usb_test.bin"
...
#define USB_DOWNLOAD_LEN        (4*1024)
#define BIT_PHY                 1
#define BIT_MAC                 (1 << 1)
#define BIT_CPU                 (1 << 2)
...
enum bt_system_state{
    W1U_BT_SYSTEM_RESUME,
    W1U_BT_SYSTEM_SUSPEND,
};
...
static w1u_usb_bt_t amlbt_dev = {0};
...
static int amlbt_submit_poll_urb(w1u_usb_bt_t *p_bt);
static void amlbt_lateresume(struct early_suspend *h);
static void amlbt_earlysuspend(struct early_suspend *h);
static int amlbt_download_firmware(w1u_usb_bt_t *p_bt);
static int amlbt_task_start(w1u_usb_bt_t *p_bt);
...
static int amlbt_check_usb(void)
{
    ...
    while ((enum usb_udev_state)g_udev->state != USB_CONFIGURED)
    {
        ...
        {
            up(&p_bt->sr_sem);//prevent use sr_sem before init.
        }
        amlbt_wakeup_lock();
        while ((enum usb_udev_state)g_udev->state != USB_CONFIGURED)
        {
...
static int amlbt_suspend_fw(w1u_usb_bt_t *p_bt)
{
    int ret = 0;
...
static void amlbt_mutex_init(w1u_usb_bt_t *p_bt)
{
    BTF("%s \n", __func__);
...
static void amlbt_mutex_deinit(w1u_usb_bt_t *p_bt)
{
    BTF("%s \n", __func__);
...
static void amlbt_fifo_deinit(gdsl_fifo_t **p_fifo, w1u_usb_bt_t *p_bt, unsigned int r_point_addr, unsigned int w_point_addr)
{
...
static void amlbt_res_deinit(w1u_usb_bt_t *p_bt)
{
    ...
    //tx queue fifo deinit
    //amlbt_hci_tx_data_deinit(p_bt);??????
...
static int amlbt_res_init(w1u_usb_bt_t *p_bt)
{
    unsigned int i = 0;
    unsigned int st_reg = 0;
...
static int amlbt_create_device(w1u_usb_bt_t *p_bt)
{
    int ret = 0;
    int cdevErr = 0;
    dev_t dev = 0;
...
static int amlbt_destroy_device(w1u_usb_bt_t *p_bt)
{
...
static int amlbt_probe(struct platform_device *dev)
{
    ...
    amlbt_input_device_init(dev);//input devices
    return 0;
}
...
static int amlbt_load_conf(w1u_usb_bt_t *p_bt)
{
    int ret = 0;
    ...
    while (pos < len) {
        ...
        *line_end = '\0';  // Null-terminate the current line
        ...
        } else if (parse_int_value(line_start, "CanSwitchSBT", &p_bt->can_switch_SBT)) {
            BTI("Parsed CanSwitchSBT: %d\n", p_bt->can_switch_SBT);
        }/* else {
...
static int amlbt_load_firmware(w1u_usb_bt_t *p_bt)
{
    int ret = 0;
    unsigned int reg = 0;
    const struct firmware *fw_entry = NULL;
    ...
    reg &= 0xeddfffff;
    reg |= ((p_bt->antenna << BIT_RF_NUM)|(p_bt->bt_sink << BT_SINK_MODE)|(p_bt->can_switch_SBT << BIT_SWITCH_STB));
    amlbt_write_word(REG_PMU_POWER_CFG, reg, USB_EP1);
    //amlbt_write_word(REG_PMU_POWER_CFG, (p_bt->antenna << BIT_RF_NUM)|(p_bt->bt_sink << BT_SINK_MODE)|(p_bt->can_switch_SBT << BIT_SWITCH_STB), USB_EP1);
...
static int amlbt_char_open(struct inode *inode, struct file *file)
{
    ...
    BTF("%s,%d, version:%s\n", __func__, amlbt_ft_mode, AML_W1UU_VERSION);
...
{
    //int ret = 0;
    w1u_usb_bt_t *p_bt = (w1u_usb_bt_t *)file->private_data;
...
*/
        ...
        ret = amlbt_aon_addr_bit_clr(RG_AON_A52, 24);
        if (ret != 0)
        {
            amlbt_res_deinit(&amlbt_dev);
            return 0;
...
#if 0
static unsigned int gdsl_read_data(gdsl_fifo_t *p_fifo, unsigned char *data, unsigned int len)
{
    ...
    if (p_fifo->w > p_fifo->r)
    {
        ...
        BTA("%s read len A %d\n", __func__, read_len);
        amlbt_read_sram(data, p_fifo->r, read_len, USB_EP1);
        p_fifo->r += read_len;
    }
    else
    {
        ...
        if (len < offset)
        {
            amlbt_read_sram(data, p_fifo->r, len, USB_EP1);
            p_fifo->r += len;
            read_len = len;
            BTA("%s 111 len %#x \n", __func__, len);
        }
        else
        {
            ...
            amlbt_read_sram(data, p_fifo->r, offset, USB_EP1);
            ...
            if (((len - offset) != 0) && (remain_len != 0))
            {
                if ((len - offset) > remain_len)
                {
                    ...
                    amlbt_read_sram(&data[offset], p_fifo->r, remain_len, USB_EP1);
                    read_len += remain_len;
                    p_fifo->r += remain_len;
                }
                else
                {
                    BTD("r2 %#x len %#x\n", p_fifo->r, remain_len);
                    amlbt_read_sram(&data[offset], p_fifo->r, len - offset, USB_EP1);
                    read_len += (len - offset);
                    p_fifo->r += (len - offset);
                }
            }
...
static int amlbt_process_data_to_skb(w1u_usb_bt_t *p_bt, unsigned char *data_buf, unsigned int len)
{
    unsigned int offset = 0;
...
static int amlbt_process_evt_to_skb(w1u_usb_bt_t *p_bt, unsigned char *event_buff, unsigned int len)
{
    unsigned int offset = 0;
...
static int amlbt_firmware_data_process(w1u_usb_bt_t *p_bt)
{
...
static int amlbt_drv_state_process(w1u_usb_bt_t *p_bt)
{
    int ret = BT_DRV_NONE;

    if (p_bt->dr_state & BT_DRV_STATE_RECOVERY)
    {
        BTF("%s:%d BT_DRV_STATE_RECOVERY!\n", __func__, __LINE__);
        p_bt->usb_irq_task_quit = 1;
        wake_up_interruptible(&p_bt->rd_wait_queue);
    }
    ...
    {
        BTF("%s:%d BT_DRV_STATE_RESUME_ENTRY!\n", __func__, __LINE__);
...
static void amlbt_polling_time_check(w1u_usb_bt_t *p_bt)
{
    if (p_bt->sink_mode)
    {
        polling_time = 1000;
...
{
    w1u_usb_bt_t *p_bt = (w1u_usb_bt_t *)data;
    int actual_length = 0;
    int ret = 0;
    int err = 0;
    ...
    while (1)
    {
        ...
        if (ret == BT_DRV_NONE && suspend_value != W1U_BT_SYSTEM_SUSPEND)
        {
        ...
        {
            BTF("%s:%d BT_DRV_SUSPEND or BT_DRV_RESUME!\n", __func__, __LINE__);
            usleep_range(polling_time, polling_time);
        }
    }
...
static int amlbt_download_firmware(w1u_usb_bt_t *p_bt)
{
    ...
    BTF("bt_usb_download_firmware:iccm size %#x\n", download_size);
    ...
    //to do download bt fw
    BTF("bt_usb_download_firmware:dccm size %#x\n", download_size);
    offset = 0;
    ...
    while (offset < download_size)
    {
        ...
        else
        {
           ret =  amlbt_write_firmware((unsigned char *)&p_bt->dccm_buf[offset], USB_DOWNLOAD_LEN, dccm_base_addr);
            ...
            BTA("bt_usb_download_firmware dccm2 offset %#x, write_len %#x\n", offset, USB_DOWNLOAD_LEN);
        }
        BTA("bt_usb_download_firmware dccm remain_len %#x \n", remain_len);
    }
    BTF("bt_usb_download success\n");
    return ret;
error:
    BTF("bt_usb_download failed\n");
    return ret;
}

static int amlbt_send_hci_cmd(w1u_usb_bt_t *p_bt, unsigned char *data, unsigned int len)
{
...
static int amlbt_send_hci_data(w1u_usb_bt_t *p_bt, unsigned char *data, unsigned int len)
{
    int ret = 0;
    unsigned int i = 0;
    ...
    //amlbt_write_sram((unsigned char *)&tx_buff[0], (unsigned char *)p_bt->tx_q[i].tx_q_prio_addr,
    //                                  sizeof(unsigned int)*4, USB_EP1);
    ret = amlbt_write_word((unsigned int)p_bt->tx_q[i].tx_q_prio_addr, tx_buff[0], USB_EP1);
    if (ret != 0)
    {
        BTE("%s:%d Failed : %d\n", __func__, __LINE__, ret);
        *p_bt->tx_q = ro_tx_q;
    ...
    ret = amlbt_write_word((unsigned int)p_bt->tx_q[i].tx_q_dev_index_addr, tx_buff[1], USB_EP1);
    if (ret != 0)
    {
        BTE("%s:%d Failed : %d\n", __func__, __LINE__, ret);
        *p_bt->tx_q = ro_tx_q;
    ...
    ret = amlbt_write_word((unsigned int)p_bt->tx_q[i].tx_q_status_addr, tx_buff[2], USB_EP1);
    if (ret != 0)
    {
        BTE("%s:%d Failed : %d\n", __func__, __LINE__, ret);
        *p_bt->tx_q = ro_tx_q;
                                     ...
                                     loff_t *pos_p)
{
    ...
    w1u_usb_bt_t *p_bt = (w1u_usb_bt_t *)file_p->private_data;
...
static int amlbt_hci_read(unsigned char *buf_p)
{
    ...
    w1u_usb_bt_t *p_bt = &amlbt_dev;//get bt struct data
...
static int amlbt_hci_close(struct hci_dev *hdev)
{
    ...
    // Clean up workqueue and other resources (if any)
    /*if (info->workqueue) {
