...
static sdio_ww_inf p_ww_sdio_inf = NULL;
static sdio_rw_inf p_rw_sdio_inf = NULL;
...
int amlbt_write_rclist_to_firmware(void)
{
    ...
    {
        if (p_ww_sdio_inf == NULL || p_rw_sdio_inf == NULL)
        {
            BTE("%s p_ww_sdio_inf or p_rw_sdio_inf is NULL\n", __func__);
            return -1;
        }
    }
    ...
    if (INTF_TYPE_IS_SDIO(amlbt_if_type))
    {
        while (offset < MAC_ADDR_LEN*MAX_MAC_LIST)
        {
            ...
            p_ww_sdio_inf(FIFO_FW_SDIO_RC_LIST_ADDR+offset, list_reg);
            offset += 4;
        }
    }
    else if (INTF_TYPE_IS_USB(amlbt_if_type))
    ...
    if (INTF_TYPE_IS_SDIO(amlbt_if_type))
    {
        ...
        while (offset < MAC_ADDR_LEN*MAX_MAC_LIST)
        {
            list_reg = p_rw_sdio_inf(FIFO_FW_SDIO_RC_LIST_ADDR+offset);
                        ...
                        sdio_rw_inf p_rw_sdio_func)
{
    ...
    else
    {
        p_ws_inf = p_ws_func;
        p_rs_inf = p_rs_func;
        p_ww_sdio_inf = p_ww_sdio_func;
        p_rw_sdio_inf = p_rw_sdio_func;
