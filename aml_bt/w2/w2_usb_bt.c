...
#define AML_BT_FIRMWARE_NAME        "aml/w2_bt_fw_usb.bin"
#define AML_BT_FIRMWARE_FT_NAME     "aml/w2_bt_fw_usb_test.bin"
#define AML_BT_FIRMWARE_TXT_NAME    "aml/w2_bt_fw_usb.txt"
...
static w2_usb_bt_t amlbt_dev = {0};
...
static int amlbt_submit_poll_urb(w2_usb_bt_t *p_bt);
static void amlbt_lateresume(struct early_suspend *h);
static void amlbt_earlysuspend(struct early_suspend *h);
static int amlbt_download_firmware(w2_usb_bt_t *p_bt);
static int amlbt_load_firmware(w2_usb_bt_t *p_bt);
static int amlbt_load_conf(w2_usb_bt_t *p_bt);
static int amlbt_res_init(w2_usb_bt_t *p_bt);
static void amlbt_res_deinit(w2_usb_bt_t *p_bt);
static int amlbt_task_start(w2_usb_bt_t *p_bt);
...
static int amlbt_suspend_fw(w2_usb_bt_t *p_bt)
{
    int ret = 0;
...
static int amlbt_write_manfdata_to_firmware(w2_usb_bt_t *p_bt)
{
    int ret = 0;
    int i = 0;
...
static int amlbt_write_macaddr_to_firmware(w2_usb_bt_t *p_bt)
{
    int ret = 0;
...
static void amlbt_mutex_init(w2_usb_bt_t *p_bt)
{
    BTF("%s \n", __func__);
...
static void amlbt_mutex_deinit(w2_usb_bt_t *p_bt)
{
    BTF("%s \n", __func__);
...
static void amlbt_fifo_deinit(gdsl_fifo_t **p_fifo, w2_usb_bt_t *p_bt, unsigned int r_point_addr, unsigned int w_point_addr)
{
...
static int amlbt_create_device(w2_usb_bt_t *p_bt)
{
    int ret = 0;
    int cdevErr = 0;
    dev_t dev = 0;
...
static int amlbt_destroy_device(w2_usb_bt_t *p_bt)
{
...
static int amlbt_register_interrupt_gpio(w2_usb_bt_t *p_bt)
{
...
static int amlbt_char_open(struct inode *inode, struct file *file)
{
    ...
    BTF("%s,%d, version:%s\n", __func__, amlbt_ft_mode, AML_W2U_VERSION);

    /*if (info->reg_flag)
    {
        amlbt_hci_dev_deinit();
        amlbt_hci_dev_init(p_bt->pdev);
        BTF("unregister hci0 than init hdev\n");
    }*/
...
{
    int ret = 0;
    w2_usb_bt_t *p_bt = (w2_usb_bt_t *)file->private_data;
    ...
    if (!amlbt_ft_mode)
    {
        ...
        /*ret = amlbt_aon_addr_bit_clr(RG_AON_A24, 24);
        if (ret != 0)
        {
            amlbt_res_deinit(&amlbt_dev);
            return 0;
...
static int amlbt_process_data_to_skb(w2_usb_bt_t *p_bt, unsigned char *data_buf, unsigned int len)
{
...
static int amlbt_process_evt_to_skb(w2_usb_bt_t *p_bt, unsigned char *event_buff, unsigned int len)
{
    unsigned int offset = 0;
...
static int amlbt_firmware_data_process(w2_usb_bt_t *p_bt)
{
...
static int amlbt_drv_state_process(w2_usb_bt_t *p_bt)
{
    int ret = BT_DRV_NONE;
...
static void amlbt_polling_time_check(w2_usb_bt_t *p_bt)
{
    if (p_bt->sink_mode)
    {
        polling_time = 1000;
...
{
    w2_usb_bt_t *p_bt = (w2_usb_bt_t *)data;
    //process data
    int actual_length = 0;
    int ret = 0;
    int err = 0;
    ...
    while (1)
    {
        ...
        if (ret == BT_DRV_NONE && suspend_value != W2_BT_SYSTEM_SUSPEND)
        {
...
static int amlbt_send_hci_cmd(w2_usb_bt_t *p_bt, unsigned char *data, unsigned int len)
{
    int ret = 0;
    unsigned int val = 0;
...
static int amlbt_send_hci_data(w2_usb_bt_t *p_bt, unsigned char *data, unsigned int len)
{
    int ret = 0;
    unsigned int i = 0;
                                     ...
                                     loff_t *pos_p)
{
    ...
    w2_usb_bt_t *p_bt = (w2_usb_bt_t *)file_p->private_data;
                                   ...
                                   loff_t *pos_p)
{
    ...
    switch (p_bt->rd_state)
    {
        case HCI_RX_TYPE:   //read type
            ...
            if (bt_type[0] != HCI_ACLDATA_PKT && bt_type[0] != HCI_EVENT_PKT)
            {
...
static int amlbt_hci_read(unsigned char *buf_p)
{
    ...
    w2_usb_bt_t *p_bt = &amlbt_dev;//get bt struct data
